...
async function gradeEnglishEssay(essay, user) {
  ...
  const levelPrompt = generateLevelPrompt(user);

  try {
    const response = await fetchGrading(apiUrl, apiKey, levelPrompt, essay);
...
function generateLevelPrompt(user) {
  return `You are an experienced and strict English teacher grading a ${user.user.level} student's essay.
...
Consider the student's level (${user.user.level}) when grading, the same essay should be marked differently for a lower grade and an upper grade student.
...
async function fetchGrading(apiUrl, apiKey, levelPrompt, essay) {
